...
describe Soracom do
  ...
  it 'initialize fails with invalid credentials' do
    expect { Soracom::Client.new(email: 'somebody@example.com', password: 'foobar') }.to raise_error('400 Bad Request')
  end

  it 'initialize succeeds with valid credentials' do
    expect { Soracom::Client.new(email: 'REPLACEME', password: 'REPLACEME') }.to raise_error('400 Bad Request')
  end
end
